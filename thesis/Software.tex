...
The last mentioned programming method is scripting in Python. We can upload the \index{MicroPython} \cite{MicroPython} firmware directly to the ESP32 controller. The Python libraries, scripts and other files are stored on the SD card. The MicroPython firmware supports most of the hardware functionality, but there are still some restrictions.
...
The \ac{API} is built on \ac{ESP-IDF} and it should be used together with the \ac{ESP-IDF} \ac{API} from the manufacturer. It allows, for example, a FreeRTOS usage, because the FreeRTOS is a part of \ac{ESP-IDF} framework \cite{espressif:ESP-WROOM-32}. The SensorBoard \ac{API} can be used together with Arduino \ac{API} for ESP32, too.
