...
public:
	...
	{
        //TODO: implement other functions, the actual code only allows charging
        readTest(0x00, data_rd, 6);
    ...
    esp_err_t readTest(uint8_t reg, uint8_t* data_rd, size_t size)
	{
	    if (size == 0) {
	        return ESP_OK;
	    }
    ...
    esp_err_t writeTest(uint8_t reg, uint8_t data)
	{
        i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	    i2c_master_start(cmd);
	    i2c_master_write_byte(cmd, 0x28, true);
